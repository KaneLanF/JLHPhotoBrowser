...
- (void)viewDidLoad {
    ...
    self.btnDone.backgroundColor = BasicColorOne;
    
    self.navigationController.fd_fullscreenPopGestureRecognizer.enabled = NO ;//关闭全屏返回功能
...
- (void)viewWillAppear:(BOOL)animated
{
    [super viewWillAppear:animated];
     ZLImageNavigationController *Nav = (ZLImageNavigationController *)self.navigationController;
    if (Nav.maxSelectCount == 1) {//单选不需要底部view操作
        return;
    }
...
#pragma mark- 单选模式不需要显示底部View,自己改动
    ZLImageNavigationController *Nav = (ZLImageNavigationController *)self.navigationController;
    if (Nav.maxSelectCount == 1) {//单选模式不需要底部view
        _bottomView.hidden = YES;
        _collectionView.height = _collectionView.height +44;
    }
}
...
- (void)resetBottomBtnsStatus
{
    ...
    if (nav.arrSelectedModels.count > 0) {
        ...
        [self.btnOriginalPhoto setTitleColor:WordColorOne forState:UIControlStateNormal];
        [self.btnPreView setTitleColor:WordColorOne forState:UIControlStateNormal];
        self.btnDone.backgroundColor = BasicColorOne;
        ...
        self.btnDone.backgroundColor = WhiteColor;
        [self.btnDone setTitleColor:WordColorThree forState:UIControlStateDisabled];
    }
    ...
    self.btnEdit.userInteractionEnabled = canEdit;
     ...
     self.btnOriginalPhoto.hidden = YES;///////////////隐藏原图按钮，避免选择大图时内存奔溃
}
...
- (void)initNavBtn
{
    ...
    [btn setTitleColor:WordColorOne forState:UIControlStateNormal];
...
- (UIViewController *)getBigImageVCWithData:(NSArray<ZLPhotoModel *> *)data index:(NSInteger)index
{
    ...
    WeakSelf
    [vc setBtnBackBlock:^(NSArray<ZLPhotoModel *> *selectedModels, BOOL isOriginal,NSArray<UIImage *> *images, NSArray<PHAsset *> *assets) {
...
-(void)scrollViewDidZoom:(UIScrollView *)scrollView
{
   ...
   ZLPhotoModel *model = self.arrDataSources[page];
    if (model.isSelected) {
        [self getOriginalImageBytes];
    } else {
        self.labPhotosBytes.text = nil;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.selectedBlock = ^(BOOL selected) {
        ...
        ZLImageNavigationController *weakNav = (ZLImageNavigationController *)weakSelf.navigationController;
        if (!selected) {
            //选中
