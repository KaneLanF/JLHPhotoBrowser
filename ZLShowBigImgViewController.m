 ...
 *  自定义导航背景view
 */
@property (nonatomic,strong) UIView * cusstomNavigationBgView;
...
- (void)viewDidLoad {
    ...
    self.currentTitleLabel.text = [NSString stringWithFormat:@"%ld/%ld", _currentPage, self.models.count];
    //[self initNavBtns];
...
-(void)initNewNaviBtns
{
    [self.view addSubview:self.cusstomNavigationBgView];
    
    //left nav btn
    UIButton *leftBtn = [ControlManager createUIButtonWithFrame:CGRectMake(0, 20, 44, 44) buttonType:UIButtonTypeCustom objc:self sel:@selector(btnBack_Click)];
    [leftBtn setImage:[UIImage imageNamed:@"navigation_bar_return_hite"] forState:UIControlStateNormal];
    [_cusstomNavigationBgView addSubview:leftBtn];
    ...
    _navRightBtn.frame = CGRectMake(ScreenWidth - 40, 0, 25, 25);
    _navRightBtn.centerY = leftBtn.centerY;
    
    UIImage *normalImg = UIImageName(@"album_photo_select");
    UIImage *selImg = UIImageName(@"album_photo_select_fil");
    ...
    [_cusstomNavigationBgView addSubview:_navRightBtn];
    ...
    [_cusstomNavigationBgView addSubview:self.currentTitleLabel];
    _currentTitleLabel.frame = CGRectMake((ScreenWidth - 200)/2, 20, 200, 44);
...
- (void)initNavBtns
{
    ...
    UIImage *navBackImg = UIImageName(@"navigation_bar_return");
...
- (void)initCollectionView
{
    ...
    if (@available(iOS 11.0, *)) {//适配iOS11系统
...
- (void)initBottomView
{
    ...
    [_btnOriginalPhoto setTitleColor:WordColorOne forState: UIControlStateNormal];
...
//    UIImage *selImg = GetImageWithName(@"btn_selected.png");
    ...
    self.labPhotosBytes.textColor = WordColorOne;
    ...
    [_btnEdit setTitleColor:WordColorOne forState:UIControlStateNormal];
    _btnEdit.frame = CGRectMake(kViewWidth/2-30, 7, 60, 30);
    ...
    [_btnDone setBackgroundColor:BasicColorOne];
    _btnDone.frame = CGRectMake(kViewWidth - 72, 5, 60, 30);
...
- (void)btnDone_Click:(UIButton *)btn
{
    ...
    if (self.arrSelPhotos && self.btnDonePreviewBlock) {
        self.btnDonePreviewBlock(self.arrSelPhotos, _arrSelAssets,nav.isSelectOriginalPhoto);
...
- (void)btnBack_Click
{
    ...
    if (self.btnBackBlock) {
        ...
        self.btnBackBlock(nav.arrSelectedModels, nav.isSelectOriginalPhoto,_arrSelPhotos, _arrSelAssets);
    }
...
- (void)resetOriginalBtnState
{
    ...
    _btnOriginalPhoto.hidden = YES;///////////////隐藏原图按钮，避免选择大图时内存奔溃
}
...
//    [[UIApplication sharedApplication] setStatusBarHidden:_hideNavBar withAnimation:UIStatusBarAnimationSlide];
    ...
    CGRect frame1 = _hideNavBar?CGRectMake(0, -NavigationBarHeight, kViewWidth, NavigationBarHeight):CGRectMake(0, 0, kViewWidth, NavigationBarHeight);
    CGRect frame = _hideNavBar?CGRectMake(0, kViewHeight, kViewWidth, 44):CGRectMake(0, kViewHeight-44, kViewWidth, 44);
    [UIView animateWithDuration:0.3 animations:^{
        _bottomView.frame = frame;
        _cusstomNavigationBgView.frame = frame1;
    }];
}

- (UIView *)cusstomNavigationBgView
{
    if (!_cusstomNavigationBgView) {
        _cusstomNavigationBgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 64)];
        _cusstomNavigationBgView.backgroundColor = [UIColor clearColor];
    }
    return _cusstomNavigationBgView;
}
- (UILabel *)currentTitleLabel
{
    if (!_currentTitleLabel) {
        _currentTitleLabel = [UILabel new];
        _currentTitleLabel.font = T4Font;
        _currentTitleLabel.textColor = WhiteColor;
        _currentTitleLabel.textAlignment = NSTextAlignmentCenter;
        _currentTitleLabel.numberOfLines = 1;
    }
