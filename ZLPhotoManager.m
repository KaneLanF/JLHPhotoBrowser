...
+ (void)saveImageToAblum:(UIImage *)image completion:(void (^)(BOOL, PHAsset *))completion
{
    ...
    if (status == PHAuthorizationStatusDenied) {
        ...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            ...
            PHAsset *asset = [weakSelf getAssetFromlocalIdentifier:placeholderAsset.localIdentifier];
            PHAssetCollection *desCollection = [weakSelf getDestinationCollection];
...
//  option.deliveryMode = PHImageRequestOptionsDeliveryModeOpportunistic;//控制照片质量
    ...
    //option.synchronous = YES;//属性yes的时候是同步请求no的时候是异步请求
    //option.resizeMode = PHImageRequestOptionsResizeModeFast;
    /*
     info字典提供请求状态信息:
