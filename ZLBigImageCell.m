...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.previewView.singleTapCallBack = ^() {
            if (weakSelf.singleTapCallBack) weakSelf.singleTapCallBack();
        };
    }
    return self;
}
...
- (void)loadGifImage:(PHAsset *)asset
{
    ...
    [ZLPhotoManager requestOriginalImageDataForAsset:asset completion:^(NSData *data, NSDictionary *info) {
        if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue]) {
            weakSelf.imageView.image = [ZLPhotoManager transformToGifImageWithData:data];
            [weakSelf resumeGif];
            [weakSelf resetSubviewSize:asset];
...
- (void)loadLivePhoto:(PHAsset *)asset
{
    ...
    [ZLPhotoManager requestLivePhotoForAsset:asset completion:^(PHLivePhoto *lv, NSDictionary *info) {
        ...
        if (lv) {
            weakSelf.lpView.livePhoto = lv;
            [weakSelf.lpView startPlaybackWithStyle:PHLivePhotoViewPlaybackStyleFull];
        }
    }];
}
    ...
    if (!_playLayer) {
        ...
        [ZLPhotoManager requestVideoForAsset:self.asset completion:^(AVPlayerItem *item, NSDictionary *info) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (!item) {
                    [weakSelf initVideoLoadFailedFromiCloudUI];
                    return;
                }
                AVPlayer *player = [AVPlayer playerWithPlayerItem:item];
                [weakSelf.layer addSublayer:weakSelf.playLayer];
                weakSelf.playLayer.player = player;
                [weakSelf switchVideoStatus];
                [weakSelf.playLayer addObserver:weakSelf forKeyPath:@"status" options:NSKeyValueObservingOptionNew context:nil];
                [[NSNotificationCenter defaultCenter] addObserver:weakSelf selector:@selector(playFinished:) name:AVPlayerItemDidPlayToEndTimeNotification object:player.currentItem];
            });
        }];
    } else {
